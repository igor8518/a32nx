...
enum GROUP_ID { GROUP_LOCAL_VARS };
...
void AddVars() {
  ...
  for (; LVarsRequestID < MAX_IDS_VARS; LVarsRequestID++) {
    ...
    if (LVar[0] != '\0') {
      ...
      if (result.first) {
        ExportVars[result.second] =
            std::make_unique<LocalVariable>(std::string(Lssstr), ID(LVarsRequestID - MIN_VAR_REQ_ID), ID(result.second), ExportVarsSet);
      } else {
        sstr = "";
      }
    ...
    case SIMCONNECT_RECV_ID_CLIENT_DATA: {
      ...
      switch (pObjData->dwRequestID) {
        case REQ_CONTROL: {
          ...
          hr = SimConnect_SetClientData(hSimConnect, A32NX_LOCAL_DATA_ID, A32NX_LOCAL_DATA_DEFINITION, 0, 0, sizeof(ExportData),
                                        &ExportData);
...
__attribute__((export_name("LVarsWrapper_gauge_callback"))) extern "C" bool LVarsWrapper_gauge_callback(FsContext ctx,
                                                                                                        int service_id,
                                                                                                        void* pData) {
  switch (service_id) {
        ...
        if (0 <= (SimConnect_Open(&hSimConnect, "L:Vars wrapper", 0, 0, 0, 0))) {
          {
            ...
            VarNames.push_back("A32NX_EFIS_L_TO_WPT_IDENT_0");
          ...
          hr =
              SimConnect_CreateClientData(hSimConnect, A32NX_LOCAL_DATA_ID, sizeof(ExportData), SIMCONNECT_CREATE_CLIENT_DATA_FLAG_DEFAULT);
          ...
          hr =
              SimConnect_CreateClientData(hSimConnect, A32NX_CONTROL_ID, sizeof(A32NX_Control), SIMCONNECT_CREATE_CLIENT_DATA_FLAG_DEFAULT);
    ...
    case PANEL_SERVICE_PRE_DRAW: {
      ...
      hr = SimConnect_SetClientData(hSimConnect, A32NX_ALLDATA_ID, A32NX_ALLDATA_DEFINITION, 0, 0,
                                    sizeof(double[MAX_OUTPUT_VARS - 1]) + sizeof(INT64), ExportVarsSet);
