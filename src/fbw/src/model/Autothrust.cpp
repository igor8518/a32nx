...
void AutothrustModelClass::Autothrust_Voter1(real_T rtu_u1, real_T rtu_u2, real_T rtu_u3, real_T *rty_Y)
{
  real_T v[3];
  int32_T rtu_u1_0;
  v[0] = rtu_u1;
...
void AutothrustModelClass::step()
{
  ...
  rtb_y_hb = std::sin(Phi_rad);
  Phi_rad = std::cos(Phi_rad);
  result_tmp[0] = rtb_Saturation;
  result_tmp[3] = 0.0;
  result_tmp[6] = -Theta_rad;
  result_tmp[1] = rtb_y_hb * Theta_rad;
  result_tmp[4] = Phi_rad;
  result_tmp[7] = rtb_Saturation * rtb_y_hb;
  result_tmp[2] = Phi_rad * Theta_rad;
  result_tmp[5] = 0.0 - rtb_y_hb;
  result_tmp[8] = Phi_rad * rtb_Saturation;
  for (rtb_on_ground = 0; rtb_on_ground < 3; rtb_on_ground++) {
    result[rtb_on_ground] = (result_tmp[rtb_on_ground + 3] * Autothrust_U.in.data.by_m_s2 + result_tmp[rtb_on_ground] *
      Autothrust_U.in.data.bx_m_s2) + result_tmp[rtb_on_ground + 6] * Autothrust_U.in.data.bz_m_s2;
  }
  ...
  Autothrust_TimeSinceCondition(Autothrust_U.in.time.simulation_time, Autothrust_U.in.input.ATHR_disconnect, &rtb_Y,
    &Autothrust_DWork.sf_TimeSinceCondition_o);
  Autothrust_DWork.Memory_PreviousInput = Autothrust_P.Logic_table[(((static_cast<uint32_T>(rtb_Y >=
  ...
  Autothrust_TimeSinceCondition(Autothrust_U.in.time.simulation_time, rtb_on_ground != 0, &rtb_Y,
  ...
  if (tmp_0 || (Autothrust_DWork.eventTime_g == 0.0)) {
    Autothrust_DWork.eventTime_g = Autothrust_U.in.time.simulation_time;
  }
  ...
  rtb_BusAssignment_n.data_computed.time_since_touchdown = rtb_Y;
  ...
  Autothrust_TLAComputation1(&rtb_BusAssignment_n, Autothrust_U.in.input.TLA_2_deg, &Phi_rad, &rtb_NOT1_m);
  ...
  } else if ((status == athr_status_ENGAGED_ARMED) && rtb_y_f && (Theta_rad == 35.0)) {
  ...
  if (Autothrust_U.in.data.is_engine_operative_1 && Autothrust_U.in.data.is_engine_operative_2) {
    rtb_y_hb = Autothrust_U.in.input.thrust_limit_CLB_percent;
  } else {
    rtb_y_hb = Autothrust_U.in.input.thrust_limit_MCT_percent;
  }

  Autothrust_Voter1(Autothrust_U.in.input.V_LS_kn, Autothrust_U.in.input.V_c_kn, Autothrust_U.in.input.V_MAX_kn,
                    &Phi_rad);
  rtb_Y_j = Phi_rad - Autothrust_U.in.data.V_ias_kn;
  rtb_y_p = Autothrust_P.Gain1_Gain_n0 * rtb_Gain2;
  Phi_rad = Autothrust_P.Gain1_Gain_o * rtb_Gain3;
  ...
  if (Autothrust_U.in.data.ambient_density_kg_per_m3 > Autothrust_P.Saturation2_UpperSat) {
    tmp = Autothrust_P.Saturation2_UpperSat;
  } else if (Autothrust_U.in.data.ambient_density_kg_per_m3 < Autothrust_P.Saturation2_LowerSat) {
    tmp = Autothrust_P.Saturation2_LowerSat;
  } else {
    tmp = Autothrust_U.in.data.ambient_density_kg_per_m3;
  }

  Autothrust_WashoutFilter(Autothrust_P._Gain * (1.0 / std::sqrt(tmp) * (Autothrust_P.GStoGS_CAS_Gain *
    (Autothrust_P.ktstomps_Gain * rtb_Switch_d))), Autothrust_P.WashoutFilter_C1, Autothrust_U.in.time.dt, &rtb_Y,
    &Autothrust_DWork.sf_WashoutFilter);
  rtb_Gain_ll = Autothrust_P.kntoms_Gain * Autothrust_U.in.data.V_gnd_kn;
  if (rtb_Gain_ll > Autothrust_P.Saturation_UpperSat_d) {
    rtb_Gain_ll = Autothrust_P.Saturation_UpperSat_d;
  } else if (rtb_Gain_ll < Autothrust_P.Saturation_LowerSat_e) {
    rtb_Gain_ll = Autothrust_P.Saturation_LowerSat_e;
  }

  Autothrust_LeadLagFilter(rtb_Y - Autothrust_P.g_Gain * (Autothrust_P.Gain1_Gain_c * (Autothrust_P.Gain_Gain_j *
    ((rtb_y_p - Autothrust_P.Gain1_Gain_j * (Autothrust_P.Gain_Gain_c * std::atan(Autothrust_P.fpmtoms_Gain *
    Autothrust_U.in.data.H_dot_fpm / rtb_Gain_ll))) * (Autothrust_P.Constant_Value - std::cos(Phi_rad)) + std::sin
     (Phi_rad) * std::sin(Autothrust_P.Gain1_Gain_d * Autothrust_U.in.data.Psi_magnetic_track_deg -
    Autothrust_P.Gain1_Gain_f * Autothrust_U.in.data.Psi_magnetic_deg)))), Autothrust_P.HighPassFilter_C1,
    Autothrust_P.HighPassFilter_C2, Autothrust_P.HighPassFilter_C3, Autothrust_P.HighPassFilter_C4,
    Autothrust_U.in.time.dt, &rtb_Switch_d, &Autothrust_DWork.sf_LeadLagFilter);
  if (Autothrust_U.in.data.V_ias_kn > Autothrust_P.Saturation1_UpperSat_c) {
    tmp = Autothrust_P.Saturation1_UpperSat_c;
  } else if (Autothrust_U.in.data.V_ias_kn < Autothrust_P.Saturation1_LowerSat_g) {
    tmp = Autothrust_P.Saturation1_LowerSat_g;
  } else {
    tmp = Autothrust_U.in.data.V_ias_kn;
  }

  Autothrust_LeadLagFilter(Autothrust_P.ktstomps_Gain_h * tmp, Autothrust_P.LowPassFilter_C1,
    Autothrust_P.LowPassFilter_C2, Autothrust_P.LowPassFilter_C3, Autothrust_P.LowPassFilter_C4, Autothrust_U.in.time.dt,
    &rtb_Y, &Autothrust_DWork.sf_LeadLagFilter_h);
  rtb_Sum5 = (rtb_Switch_d + rtb_Y) * Autothrust_P.ktstomps1_Gain * Autothrust_P.Gain4_Gain * look1_binlxpw(rtb_Y_j,
    Autothrust_P.ScheduledGain1_BreakpointsForDimension1, Autothrust_P.ScheduledGain1_Table, 4U) + rtb_Y_j;
  rtb_Gain_ll = Autothrust_P.DiscreteDerivativeVariableTs_Gain * rtb_Sum5;
  Autothrust_LagFilter((rtb_Gain_ll - Autothrust_DWork.Delay_DSTATE) / Autothrust_U.in.time.dt,
                       Autothrust_P.LagFilter_C1, Autothrust_U.in.time.dt, &rtb_Y, &Autothrust_DWork.sf_LagFilter);
  ...
  rtb_Gain2 = rtb_Y_j - std::cos(Autothrust_P.Gain1_Gain_p * rtb_Gain2) / std::cos(Autothrust_P.Gain1_Gain_di *
    rtb_Gain3);
  Autothrust_WashoutFilter(Autothrust_P.Gain2_Gain_c * rtb_Gain2, Autothrust_P.WashoutFilter_C1_c,
    Autothrust_U.in.time.dt, &rtb_y_p, &Autothrust_DWork.sf_WashoutFilter_h);
  Phi_rad = Autothrust_U.in.input.is_alt_soft_mode_active;
  Autothrust_Voter1(Autothrust_U.in.input.V_LS_kn, Autothrust_U.in.input.V_c_kn, Autothrust_U.in.input.V_MAX_kn,
                    &Phi_rad);
  Autothrust_Voter1(Autothrust_U.in.input.V_LS_kn, Autothrust_U.in.input.V_c_kn, Autothrust_U.in.input.V_MAX_kn,
                    &rtb_Switch_d);
  if (Autothrust_U.in.input.mode_requested > Autothrust_P.Saturation_UpperSat_l) {
    tmp = Autothrust_P.Saturation_UpperSat_l;
  } else if (Autothrust_U.in.input.mode_requested < Autothrust_P.Saturation_LowerSat_i) {
    tmp = Autothrust_P.Saturation_LowerSat_i;
  } else {
    tmp = Autothrust_U.in.input.mode_requested;
  }

  switch (static_cast<int32_T>(tmp)) {
   case 0:
    Phi_rad = Autothrust_P.Constant1_Value;
    break;

   case 1:
    Phi_rad = ((Autothrust_P.Gain_Gain * rtb_Sum5 + rtb_Y) + (Autothrust_P.Gain1_Gain * rtb_Gain2 +
                Autothrust_P.Gain3_Gain * rtb_y_p)) * look1_binlxpw(std::fmin
    ...
    if (Phi_rad > Autothrust_P.Saturation_UpperSat) {
      Phi_rad = Autothrust_P.Saturation_UpperSat;
    } else if (Phi_rad < Autothrust_P.Saturation_LowerSat) {
      Phi_rad = Autothrust_P.Saturation_LowerSat;
    }
    break;

   case 2:
    Phi_rad = Autothrust_P.THRIDLE_Value - std::abs((Phi_rad - Autothrust_U.in.data.V_ias_kn) * Autothrust_P.Gain_Gain_f);
    if (Phi_rad > Autothrust_P.Saturation_UpperSat_n) {
      Phi_rad = Autothrust_P.Saturation_UpperSat_n;
    } else if (Phi_rad < Autothrust_P.Saturation_LowerSat_j) {
      Phi_rad = Autothrust_P.Saturation_LowerSat_j;
    }
    break;

   case 3:
    Phi_rad = (rtb_Switch_d - Autothrust_U.in.data.V_ias_kn) * Autothrust_P.Gain_Gain_m + (look1_binlxpw(std::fmin
      (rtb_BusAssignment_n.data.commanded_engine_N1_1_percent, rtb_BusAssignment_n.data.commanded_engine_N1_1_percent),
      Autothrust_P.uDLookupTable_bp01Data, Autothrust_P.uDLookupTable_tableData, 4U) + Autothrust_P.THRCLBMCT_Value);
    if (Phi_rad > Autothrust_P.Saturation_UpperSat_a) {
      Phi_rad = Autothrust_P.Saturation_UpperSat_a;
    } else if (Phi_rad < Autothrust_P.Saturation_LowerSat_a) {
      Phi_rad = Autothrust_P.Saturation_LowerSat_a;
    }
    break;

   default:
    Phi_rad = Autothrust_P.RETARD_Value;
    break;
  }

  Phi_rad = Autothrust_P.DiscreteTimeIntegratorVariableTsLimit_Gain * Phi_rad * Autothrust_U.in.time.dt;
  ...
  if (Autothrust_DWork.icLoad) {
    Autothrust_DWork.Delay_DSTATE_k = std::fmax(rtb_BusAssignment_n.data.commanded_engine_N1_1_percent,
      rtb_BusAssignment_n.data.commanded_engine_N1_2_percent) - Phi_rad;
  }

  Autothrust_DWork.Delay_DSTATE_k += Phi_rad;
  if (Autothrust_DWork.Delay_DSTATE_k > rtb_y_hb) {
    Autothrust_DWork.Delay_DSTATE_k = rtb_y_hb;
  ...
  Phi_rad = rtb_Gain2 - rtb_BusAssignment_n.data.commanded_engine_N1_1_percent;
  if (rtb_Compare_e) {
    Autothrust_DWork.Delay_DSTATE_n = Autothrust_P.DiscreteTimeIntegratorVariableTs_InitialCondition;
  }

  Autothrust_DWork.Delay_DSTATE_n += Autothrust_P.Gain_Gain_d3 * Phi_rad *
  ...
  Autothrust_DWork.Delay_DSTATE_l += Autothrust_P.Gain1_Gain_h * Phi_rad *
  ...
  Autothrust_ThrustMode1(Autothrust_U.in.input.TLA_1_deg, &rtb_y_hb);
  rtb_Switch_d = rtb_Gain3 - rtb_BusAssignment_n.data.commanded_engine_N1_2_percent;
  if (rtb_NOT1_m) {
    Autothrust_DWork.Delay_DSTATE_lz = Autothrust_P.DiscreteTimeIntegratorVariableTs_InitialCondition_n;
  }

  rtb_y_p = Autothrust_P.Gain_Gain_b * rtb_Switch_d * Autothrust_P.DiscreteTimeIntegratorVariableTs_Gain_k *
    Autothrust_U.in.time.dt + Autothrust_DWork.Delay_DSTATE_lz;
  if (rtb_y_p > Autothrust_P.DiscreteTimeIntegratorVariableTs_UpperLimit_p) {
    Autothrust_DWork.Delay_DSTATE_lz = Autothrust_P.DiscreteTimeIntegratorVariableTs_UpperLimit_p;
  } else if (rtb_y_p < Autothrust_P.DiscreteTimeIntegratorVariableTs_LowerLimit_e) {
    Autothrust_DWork.Delay_DSTATE_lz = Autothrust_P.DiscreteTimeIntegratorVariableTs_LowerLimit_e;
  } else {
    Autothrust_DWork.Delay_DSTATE_lz = rtb_y_p;
  }
  ...
  Autothrust_Y.out.output.sim_thrust_mode_1 = rtb_y_hb;
  ...
  Autothrust_DWork.Delay_DSTATE = rtb_Gain_ll;
