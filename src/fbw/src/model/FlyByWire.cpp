...
void FlyByWireModelClass::step()
{
  ...
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch1_Threshold_ke) {
    ...
    if (FlyByWire_DWork.sProtActive > FlyByWire_P.Switch2_Threshold) {
      rtb_Y_p = (((((rtb_v_target - FlyByWire_U.in.data.V_ias_kn) * FlyByWire_P.Gain6_Gain +
                  ...
                  rtb_qk) + FlyByWire_P.qk_dot_gain1_Gain * rtb_Y_fp) * FlyByWire_P.HSP_gain_Gain;
      ...
      if (rtb_Y_p > FlyByWire_P.Saturation4_UpperSat) {
        rtb_Y_p = FlyByWire_P.Saturation4_UpperSat;
      } else if (rtb_Y_p < FlyByWire_P.Saturation4_LowerSat) {
        rtb_Y_p = FlyByWire_P.Saturation4_LowerSat;
      }

      rtb_Switch_c += rtb_Y_p;
    } else {
    ...
    rtb_Y_p = FlyByWire_P.Gain2_Gain * rtb_Y_k - rtb_Gain1_b;
    if (rtb_Y_p > FlyByWire_P.Saturation1_UpperSat) {
      rtb_Y_p = FlyByWire_P.Saturation1_UpperSat;
    } else if (rtb_Y_p < FlyByWire_P.Saturation1_LowerSat) {
      rtb_Y_p = FlyByWire_P.Saturation1_LowerSat;
    }

    rtb_Y_g = look1_binlxpw(rtb_Y_p, FlyByWire_P.Loaddemand1_bp01Data, FlyByWire_P.Loaddemand1_tableData, 2U) +
      rtb_Y_mc5;
    if (rtb_Loaddemand <= rtb_Y_g) {
      rtb_Y_p = FlyByWire_P.Gain3_Gain * FlyByWire_P.Theta_max3_Value - rtb_Gain1_b;
      if (rtb_Y_p > FlyByWire_P.Saturation2_UpperSat) {
        rtb_Y_p = FlyByWire_P.Saturation2_UpperSat;
      } else if (rtb_Y_p < FlyByWire_P.Saturation2_LowerSat) {
        rtb_Y_p = FlyByWire_P.Saturation2_LowerSat;
      }

      rtb_Y_g = look1_binlxpw(rtb_Y_p, FlyByWire_P.Loaddemand2_bp01Data, FlyByWire_P.Loaddemand2_tableData, 2U) +
        rtb_Y_mc5;
      if (rtb_Loaddemand >= rtb_Y_g) {
  ...
  rtb_Switch_c = (rtb_Y_k1 - rtb_Y_c) * rtb_Divide_ke;
  ...
  FlyByWire_DWork.pU = rtb_Divide_ke;
  rtb_Limiterxi = (rtb_Switch_c - (rtb_Y_p - rtb_Y_c)) - FlyByWire_DWork.pY_p;
  ...
  rtb_Y_p = (((FlyByWire_P.precontrol_gain_Gain * FlyByWire_DWork.Delay1_DSTATE_o + rtb_alpha_err_gain) +
  ...
  if (rtb_Y_p > FlyByWire_P.Saturation3_UpperSat_c) {
    rtb_Y_p = FlyByWire_P.Saturation3_UpperSat_c;
  } else if (rtb_Y_p < FlyByWire_P.Saturation3_LowerSat_h) {
    rtb_Y_p = FlyByWire_P.Saturation3_LowerSat_h;
  }
  ...
  rtb_Limiterxi = (FlyByWire_P.Constant_Value_p - rtb_Limiterxi1) * rtb_Limiterxi + rtb_Y_p * rtb_Limiterxi1;
  ...
  FlyByWire_LagFilter(FlyByWire_U.in.data.engine_2_thrust_lbf - FlyByWire_U.in.data.engine_1_thrust_lbf,
                      FlyByWire_P.LagFilter1_C1_j, FlyByWire_U.in.time.dt, &rtb_Y_fp, &FlyByWire_DWork.sf_LagFilter_f);
  ...
  rtb_Y_p = (rtb_Gain - std::sin(FlyByWire_P.Gain1_Gain_f * rtb_uDLookupTable_g) * FlyByWire_P.Constant2_Value_p * std::
             cos(FlyByWire_P.Gain1_Gain_l * rtb_GainTheta) / (FlyByWire_P.Gain6_Gain_k * L_xi) *
  ...
  if (rtb_Y_p > FlyByWire_P.Saturation1_UpperSat_j) {
    rtb_Y_p = FlyByWire_P.Saturation1_UpperSat_j;
  } else if (rtb_Y_p < FlyByWire_P.Saturation1_LowerSat_a) {
    rtb_Y_p = FlyByWire_P.Saturation1_LowerSat_a;
  }
  ...
  rtb_Limiterxi1 = (FlyByWire_P.Constant_Value_ku - rtb_Limiterxi1) * rtb_Limiterxi + rtb_Y_p * rtb_Limiterxi1;
  ...
  rtb_Y_p += rtb_Gain1_b * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn,
    FlyByWire_P.ScheduledGain_BreakpointsForDimension1_c, FlyByWire_P.ScheduledGain_Table_d, 8U);
  if (rtb_Y_p > FlyByWire_P.Saturation_UpperSat_p4) {
    rtb_Y_p = FlyByWire_P.Saturation_UpperSat_p4;
  } else if (rtb_Y_p < FlyByWire_P.Saturation_LowerSat_he) {
    rtb_Y_p = FlyByWire_P.Saturation_LowerSat_he;
  }

  rtb_uDLookupTable_g = (rtb_Limiterxi * rtb_Y_p + rtb_uDLookupTable_g) + rtb_Limiterxi1;
  ...
  Vtas = (FlyByWire_P.Constant_Value_l - rtb_Gain1_b) * rtb_Y_nl + FlyByWire_DWork.Delay_DSTATE_eu * rtb_Gain1_b;
  ...
  FlyByWire_RateLimiter(Vtas, FlyByWire_P.RateLimiterxi_up, FlyByWire_P.RateLimiterxi_lo, FlyByWire_U.in.time.dt,
                        FlyByWire_P.RateLimiterxi_InitialCondition, &L_xi, &FlyByWire_DWork.sf_RateLimiter_h);
  ...
  rtb_Y_p = FlyByWire_P.Gain_Gain_i * FlyByWire_U.in.data.gear_animation_pos_0 - FlyByWire_P.Constant_Value_g;
  if (rtb_Y_p > FlyByWire_P.Saturation_UpperSat_e) {
    FlyByWire_Y.out.sim.data.gear_strut_compression_0 = FlyByWire_P.Saturation_UpperSat_e;
  } else if (rtb_Y_p < FlyByWire_P.Saturation_LowerSat_e) {
    FlyByWire_Y.out.sim.data.gear_strut_compression_0 = FlyByWire_P.Saturation_LowerSat_e;
  } else {
    FlyByWire_Y.out.sim.data.gear_strut_compression_0 = rtb_Y_p;
  }
  ...
  FlyByWire_Y.out.pitch.law_normal.protection_alpha_c_deg = rtb_Y_c + rtb_Switch_c;
  ...
  u0 = FlyByWire_P.Gainxi_Gain_n * L_xi;
