...
void FlyByWireModelClass::step()
{
  ...
  rtb_Tsxlo = FlyByWire_P.Gainpk2_Gain * FlyByWire_U.in.data.eta_trim_deg;
  ...
  rtb_Y_lp = FlyByWire_P.Gaineta_Gain * FlyByWire_U.in.input.delta_eta_pos;
  ...
  rtb_BusAssignment_sim_input_delta_eta_pos = rtb_Y_lp;
  ...
  FlyByWire_eta_trim_limit_lofreeze(rtb_Tsxlo, FlyByWire_DWork.sProtActive_c, &rtb_Y_p,
  ...
  FlyByWire_eta_trim_limit_lofreeze(rtb_Tsxlo, FlyByWire_DWork.sProtActive, &rtb_Y_p,
  ...
  if (FlyByWire_DWork.is_active_c8_FlyByWire == 0U) {
    ...
    rtb_eta_trim_deg_reset_deg = rtb_Tsxlo;
    ...
    switch (FlyByWire_DWork.is_c8_FlyByWire) {
     ...
     case FlyByWire_IN_reset:
      if ((FlyByWire_B.in_flight == 0.0) && (rtb_Tsxlo == 0.0)) {
        FlyByWire_DWork.is_c8_FlyByWire = FlyByWire_IN_manual;
        rtb_eta_trim_deg_reset = true;
        rtb_eta_trim_deg_reset_deg = rtb_Tsxlo;
        rtb_eta_trim_deg_should_write = false;
  ...
  rtb_Y_lp = (((((rtb_Gain_c - FlyByWire_DWork.Delay_DSTATE_dd) / FlyByWire_U.in.time.dt + FlyByWire_P.Gain_Gain_h *
  ...
  if (FlyByWire_DWork.icLoad) {
    FlyByWire_DWork.Delay_DSTATE_e = FlyByWire_P.Constant_Value_h - rtb_Y_lp;
  }

  FlyByWire_DWork.Delay_DSTATE_e += rtb_Y_lp;
  ...
  if (rtb_on_ground > FlyByWire_P.Switch_Threshold_he) {
    if (rtb_BusAssignment_cs_pitch_data_computed_delta_eta_deg > FlyByWire_P.Saturation_UpperSat_g) {
      rtb_Y_lp = FlyByWire_P.Saturation_UpperSat_g;
    } else if (rtb_BusAssignment_cs_pitch_data_computed_delta_eta_deg < FlyByWire_P.Saturation_LowerSat_p) {
      rtb_Y_lp = FlyByWire_P.Saturation_LowerSat_p;
    } else {
      rtb_Y_lp = rtb_BusAssignment_cs_pitch_data_computed_delta_eta_deg;
    }
  } else {
    rtb_Y_lp = FlyByWire_P.Constant1_Value_i;
  }

  rtb_Limitereta = FlyByWire_DWork.Delay_DSTATE_e + rtb_Y_lp;
  rtb_Y_lp = std::cos(FlyByWire_P.Gain1_Gain_p * rtb_GainTheta);
  rtb_uDLookupTable_g = rtb_Y_lp / std::cos(FlyByWire_P.Gain1_Gain_pa * rtb_GainPhi);
  rtb_Limiterxi = FlyByWire_P.Gain1_Gain_j * rtb_qk * (FlyByWire_P.Gain_Gain_dc * FlyByWire_P.Vm_currentms_Value) +
    (FlyByWire_U.in.data.nz_g - rtb_uDLookupTable_g);
  ...
  if (rtb_GainPhi > FlyByWire_P.Saturation_UpperSat_d) {
    ...
    rtb_Gain1_b = FlyByWire_P.Saturation_LowerSat_pr;
  } else {
    rtb_Gain1_b = rtb_GainPhi;
  }

  rtb_Divide_ke = rtb_Y_lp / std::cos(FlyByWire_P.Gain1_Gain_b * rtb_Gain1_b);
  ...
  rtb_Y_lp = FlyByWire_P.Gain2_Gain_g * rtb_Y_k - rtb_Gain1_b;
  if (rtb_Y_lp > FlyByWire_P.Saturation1_UpperSat_h) {
    rtb_Y_lp = FlyByWire_P.Saturation1_UpperSat_h;
  } else if (rtb_Y_lp < FlyByWire_P.Saturation1_LowerSat_o) {
    rtb_Y_lp = FlyByWire_P.Saturation1_LowerSat_o;
  }
  ...
  if (rtb_Y_g > rtb_Y_lp) {
    ...
    rtb_Gain1_b = FlyByWire_P.Gain3_Gain * FlyByWire_P.Theta_max3_Value - rtb_Gain1_b;
    if (rtb_Gain1_b > FlyByWire_P.Saturation2_UpperSat) {
      rtb_Gain1_b = FlyByWire_P.Saturation2_UpperSat;
    } else if (rtb_Gain1_b < FlyByWire_P.Saturation2_LowerSat) {
      rtb_Gain1_b = FlyByWire_P.Saturation2_LowerSat;
    }

    if (rtb_Y_g < rtb_Gain1_b) {
      rtb_Y_g = rtb_Gain1_b;
    }
  }

  rtb_Gain1_b = FlyByWire_P.Subsystem2_Gain * rtb_v_target;
  rtb_Y_k = (rtb_Gain1_b - FlyByWire_DWork.Delay_DSTATE_j) / FlyByWire_U.in.time.dt;
  rtb_Y_lp = FlyByWire_U.in.time.dt * FlyByWire_P.Subsystem2_C1;
  rtb_Limiterxi1 = rtb_Y_lp + FlyByWire_P.Constant_Value_m3;
  FlyByWire_DWork.Delay1_DSTATE = 1.0 / rtb_Limiterxi1 * (FlyByWire_P.Constant_Value_m3 - rtb_Y_lp) *
    FlyByWire_DWork.Delay1_DSTATE + (rtb_Y_k + FlyByWire_DWork.Delay_DSTATE_c) * (rtb_Y_lp / rtb_Limiterxi1);
  rtb_Gain_eu = FlyByWire_P.Subsystem_Gain * FlyByWire_U.in.data.V_ias_kn;
  rtb_Divide_o = (rtb_Gain_eu - FlyByWire_DWork.Delay_DSTATE_p) / FlyByWire_U.in.time.dt;
  rtb_Y_lp = FlyByWire_U.in.time.dt * FlyByWire_P.Subsystem_C1;
  rtb_Limiterxi1 = rtb_Y_lp + FlyByWire_P.Constant_Value_hz;
  FlyByWire_DWork.Delay1_DSTATE_i = 1.0 / rtb_Limiterxi1 * (FlyByWire_P.Constant_Value_hz - rtb_Y_lp) *
    FlyByWire_DWork.Delay1_DSTATE_i + (rtb_Divide_o + FlyByWire_DWork.Delay_DSTATE_m) * (rtb_Y_lp / rtb_Limiterxi1);
  ...
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch1_Threshold_ke) {
    rtb_Y_lp = (rtb_Y_mc5 - rtb_GainTheta) * FlyByWire_P.Gain4_Gain;
  } else {
    ...
    rtb_Y_lp = ((FlyByWire_P.Constant_Value_k - rtb_Saturation_kd) * rtb_Y_mc5 + rtb_Switch2_j * rtb_Saturation_kd) +
      rtb_Switch_c;
  }

  rtb_Y_lp += rtb_Divide_ke;
  if (rtb_Y_lp > rtb_Y_jz) {
    rtb_Y_lp = rtb_Y_jz;
  } else if (rtb_Y_lp < rtb_Y_lc) {
    rtb_Y_lp = rtb_Y_lc;
  }

  rtb_y_l = (rtb_Y_k1 - rtb_Y_c) * rtb_Y_g;
  ...
  rtb_Limiterxi1 = rtb_Y_p - rtb_Y_c;
  ...
  rtb_Limiterxi1 = (rtb_y_l - rtb_Limiterxi1) - rtb_Y_p;
  rtb_Divide_ke = FlyByWire_P.Subsystem1_Gain * rtb_Limiterxi1;
  rtb_Y_g = (rtb_Divide_ke - FlyByWire_DWork.Delay_DSTATE_ps) / FlyByWire_U.in.time.dt;
  rtb_Limiterxi2 = FlyByWire_U.in.time.dt * FlyByWire_P.Subsystem1_C1;
  rtb_Y_mc5 = rtb_Limiterxi2 + FlyByWire_P.Constant_Value_kr;
  FlyByWire_DWork.Delay1_DSTATE_o = 1.0 / rtb_Y_mc5 * (FlyByWire_P.Constant_Value_kr - rtb_Limiterxi2) *
    FlyByWire_DWork.Delay1_DSTATE_o + (rtb_Y_g + FlyByWire_DWork.Delay_DSTATE_c1) * (rtb_Limiterxi2 / rtb_Y_mc5);
  rtb_alpha_err_gain = FlyByWire_P.alpha_err_gain_Gain * rtb_Limiterxi1;
  rtb_Y_mc5 = FlyByWire_P.Subsystem3_Gain * FlyByWire_U.in.data.V_ias_kn;
  rtb_Switch_c = (rtb_Y_mc5 - FlyByWire_DWork.Delay_DSTATE_l) / FlyByWire_U.in.time.dt;
  rtb_Limiterxi1 = FlyByWire_U.in.time.dt * FlyByWire_P.Subsystem3_C1;
  rtb_Limiterxi2 = rtb_Limiterxi1 + FlyByWire_P.Constant_Value_c;
  FlyByWire_DWork.Delay1_DSTATE_n = 1.0 / rtb_Limiterxi2 * (FlyByWire_P.Constant_Value_c - rtb_Limiterxi1) *
    FlyByWire_DWork.Delay1_DSTATE_n + (rtb_Switch_c + FlyByWire_DWork.Delay_DSTATE_n) * (rtb_Limiterxi1 / rtb_Limiterxi2);
  ...
  if (FlyByWire_DWork.Delay_DSTATE_k > FlyByWire_P.Saturation_UpperSat_a) {
    rtb_Limiterxi2 = FlyByWire_P.Saturation_UpperSat_a;
  } else if (FlyByWire_DWork.Delay_DSTATE_k < FlyByWire_P.Saturation_LowerSat_ps) {
    rtb_Limiterxi2 = FlyByWire_P.Saturation_LowerSat_ps;
  } else {
    rtb_Limiterxi2 = FlyByWire_DWork.Delay_DSTATE_k;
  }

  rtb_Saturation_kd = FlyByWire_P.DiscreteDerivativeVariableTs1_Gain * rtb_qk;
  ...
  rtb_uDLookupTable_g = rtb_Limiterxi - (look1_binlxpw(FlyByWire_U.in.data.V_tas_kn,
    FlyByWire_P.uDLookupTable_bp01Data_j, FlyByWire_P.uDLookupTable_tableData_l, 6U) / (FlyByWire_P.Gain5_Gain_g * L_xi)
    + FlyByWire_P.Bias_Bias) * (rtb_Y_lp - rtb_uDLookupTable_g);
  rtb_Switch2_j = rtb_uDLookupTable_g * look1_binlxpw(FlyByWire_U.in.data.V_tas_kn, FlyByWire_P.DLUT_bp01Data,
    FlyByWire_P.DLUT_tableData, 1U) * FlyByWire_P.DiscreteDerivativeVariableTs_Gain_e;
  rtb_Gain_n = FlyByWire_P.DiscreteDerivativeVariableTs2_Gain * FlyByWire_U.in.data.V_tas_kn;
  FlyByWire_LagFilter((rtb_Gain_n - FlyByWire_DWork.Delay_DSTATE_fi) / FlyByWire_U.in.time.dt,
  ...
  if (rtb_Y_p > FlyByWire_P.SaturationV_dot_UpperSat) {
    rtb_Gain1_o = FlyByWire_P.SaturationV_dot_UpperSat;
  } else if (rtb_Y_p < FlyByWire_P.SaturationV_dot_LowerSat) {
    rtb_Gain1_o = FlyByWire_P.SaturationV_dot_LowerSat;
  } else {
    rtb_Gain1_o = rtb_Y_p;
  }
  ...
  rtb_uDLookupTable_g = ((((rtb_Saturation_kd - FlyByWire_DWork.Delay_DSTATE_ca) / FlyByWire_U.in.time.dt *
    FlyByWire_P.Gain3_Gain_l + rtb_uDLookupTable_g * look1_binlxpw(FlyByWire_U.in.data.V_tas_kn,
    FlyByWire_P.PLUT_bp01Data, FlyByWire_P.PLUT_tableData, 1U)) + (rtb_Switch2_j - FlyByWire_DWork.Delay_DSTATE_jv) /
    FlyByWire_U.in.time.dt) + FlyByWire_P.Gain_Gain_o * rtb_Gain1_o) + rtb_Y_p * look1_binlxpw
    (FlyByWire_U.in.data.H_radio_ft, FlyByWire_P.ScheduledGain_BreakpointsForDimension1, FlyByWire_P.ScheduledGain_Table,
     3U);
  ...
  if (rtb_uDLookupTable_g > FlyByWire_P.Saturation_UpperSat_j) {
    rtb_uDLookupTable_g = FlyByWire_P.Saturation_UpperSat_j;
  } else if (rtb_uDLookupTable_g < FlyByWire_P.Saturation_LowerSat_c) {
    rtb_uDLookupTable_g = FlyByWire_P.Saturation_LowerSat_c;
  }

  rtb_uDLookupTable_g = (FlyByWire_P.Constant_Value_p - rtb_Limiterxi2) * rtb_uDLookupTable_g + L_xi * rtb_Limiterxi2;
  FlyByWire_Y.out.pitch.law_normal.nz_c_g = rtb_Y_lp;
  rtb_Limiterxi2 = rtb_uDLookupTable_g * look1_binlxpw(FlyByWire_U.in.time.dt,
    FlyByWire_P.ScheduledGain_BreakpointsForDimension1_c, FlyByWire_P.ScheduledGain_Table_p, 4U);
  FlyByWire_Y.out.pitch.law_normal.eta_dot_deg_s = rtb_uDLookupTable_g;
  rtb_Y_lp = FlyByWire_P.DiscreteTimeIntegratorVariableTs_Gain_k * rtb_Limiterxi2 * FlyByWire_U.in.time.dt;
  ...
  if (FlyByWire_DWork.icLoad_e) {
    ...
    FlyByWire_DWork.Delay_DSTATE_f1 = L_xi - rtb_Y_lp;
  }
  ...
  if (rtb_Y_f > FlyByWire_P.Saturation_UpperSat_g4) {
    ...
    rtb_Y_lp = FlyByWire_P.Saturation_LowerSat_l;
  } else {
    rtb_Y_lp = rtb_Y_f;
  }

  rtb_uDLookupTable_g = FlyByWire_DWork.Delay_DSTATE_f1 * rtb_Y_lp;
  rtb_Y_fp = FlyByWire_P.Constant_Value_o - rtb_Y_lp;
  ...
  rtb_alpha_err_gain = ((FlyByWire_P.Constant_Value_ju - rtb_Y_lp) *
                        rtb_BusAssignment_cs_pitch_data_computed_delta_eta_deg + rtb_Limitereta * rtb_Y_lp) * rtb_Y_fp +
    rtb_uDLookupTable_g;
  ...
  if (FlyByWire_DWork.icLoad_i) {
    FlyByWire_DWork.Delay_DSTATE_h = rtb_eta_trim_deg_reset_deg - rtb_Y_lp;
  }

  FlyByWire_DWork.Delay_DSTATE_h += rtb_Y_lp;
  ...
  if ((FlyByWire_U.in.data.V_ias_kn <= 400.0) && (FlyByWire_U.in.data.V_ias_kn >= 0.0)) {
    ...
    rtb_Limiterxi1 = (FlyByWire_U.in.data.V_ias_kn - static_cast<real_T>(b[low_i])) / static_cast<real_T>(b[low_i + 1] -
      b[low_i]);
    if (rtb_Limiterxi1 == 0.0) {
      rtb_Limiterxi1 = c[low_i];
    } else if (rtb_Limiterxi1 == 1.0) {
      rtb_Limiterxi1 = c[low_i + 1];
    } else if (c[low_i + 1] == c[low_i]) {
      rtb_Limiterxi1 = c[low_i];
    } else {
      rtb_Limiterxi1 = (1.0 - rtb_Limiterxi1) * static_cast<real_T>(c[low_i]) + static_cast<real_T>(c[low_i + 1]) *
        rtb_Limiterxi1;
    }
  }

  k_phi = -(rtb_Limiterxi1 * rtb_Limiterxi1) / L_xi;
  FlyByWire_DWork.Delay_DSTATE_eu = ((-(rtb_Y_p / rtb_Limiterxi * 122.0 * 320.40999999999997 * -0.487 / 1.0E+6 + 1.414 *
    rtb_Limiterxi1) / L_xi * (FlyByWire_P.Gain1_Gain_cb * rtb_pk) + FlyByWire_P.Gain1_Gain_bqd * rtb_GainPhi * k_phi) +
    FlyByWire_P.Gain1_Gain_n * rtb_uDLookupTable_g * -k_phi) * look1_binlxpw(FlyByWire_U.in.time.dt,
  ...
  rtb_Limiterxi1 = (FlyByWire_P.Constant_Value_ku - rtb_Limiterxi1) * rtb_uDLookupTable_g + L_xi * rtb_Limiterxi1;
  ...
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch2_Threshold_n) {
    ...
    rtb_Y_lp = rtb_BusAssignment_sim_input_delta_zeta_pos * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn,
  ...
  if (FlyByWire_U.in.data.V_ias_kn >= 60.0) {
    ...
    Vtas = (((rtb_Y_p * 0.814 * FlyByWire_U.in.data.beta_deg * 3.1415926535897931 / 180.0 + -(rtb_Gain *
               3.1415926535897931 / 180.0)) + k_phi * 3.1415926535897931 / 180.0 * (9.81 / Vtas)) + rtb_Y_p * 3.172 *
  ...
  FlyByWire_LagFilter((rtb_Y_lp - L_xi) * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn,
  ...
  L_xi = rtb_Y_lp * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn, FlyByWire_P.ScheduledGain_BreakpointsForDimension1_cf,
  ...
  if (rtb_Limiterxi > FlyByWire_P.Saturation_UpperSat_i) {
    rtb_Y_lp = FlyByWire_P.Saturation_UpperSat_i;
  } else if (rtb_Limiterxi < FlyByWire_P.Saturation_LowerSat_f) {
    rtb_Y_lp = FlyByWire_P.Saturation_LowerSat_f;
  } else {
    rtb_Y_lp = rtb_Limiterxi;
  }

  rtb_Limiterxi = (FlyByWire_P.Constant_Value_f - rtb_Y_lp) * rtb_Gain1_o + rtb_uDLookupTable_g * rtb_Y_lp;
  ...
  rtb_Y_lp = FlyByWire_P.Gain4_Gain_e * L_xi * FlyByWire_P.DiscreteTimeIntegratorVariableTs1_Gain *
    FlyByWire_U.in.time.dt;
  ...
  FlyByWire_RateLimiter(rtb_alpha_err_gain, FlyByWire_P.RateLimitereta_up, FlyByWire_P.RateLimitereta_lo,
  ...
  FlyByWire_Y.out.sim.data.eta_trim_deg = rtb_Tsxlo;
  ...
  FlyByWire_Y.out.pitch.vote.eta_dot_deg_s = rtb_Limiterxi2;
  FlyByWire_Y.out.pitch.integrated.eta_deg = FlyByWire_DWork.Delay_DSTATE_f1;
  FlyByWire_Y.out.pitch.output.eta_deg = rtb_alpha_err_gain;
  ...
  FlyByWire_Y.out.roll.law_normal.Phi_c_deg = k_phi;
  ...
  if (rtb_GainPhi <= 380.0) {
    ...
    rtb_Limiterxi1 = (rtb_GainPhi - static_cast<real_T>(b_0[low_i - 1])) / static_cast<real_T>(b_0[low_i] - b_0[low_i -
      1]);
    if (rtb_Limiterxi1 == 0.0) {
      rtb_GainTheta = -15.0;
    } else if (rtb_Limiterxi1 == 1.0) {
      ...
      rtb_GainTheta = (1.0 - rtb_Limiterxi1) * -15.0 + rtb_Limiterxi1 * static_cast<real_T>(c_0[low_i]);
    }
  }
  ...
  FlyByWire_DWork.Delay_DSTATE_j = rtb_Gain1_b;
  FlyByWire_DWork.Delay_DSTATE_c = rtb_Y_k;
  FlyByWire_DWork.Delay_DSTATE_p = rtb_Gain_eu;
  ...
  FlyByWire_DWork.Delay_DSTATE_n = rtb_Switch_c;
  FlyByWire_DWork.Delay_DSTATE_ca = rtb_Saturation_kd;
  FlyByWire_DWork.Delay_DSTATE_jv = rtb_Switch2_j;
  FlyByWire_DWork.Delay_DSTATE_fi = rtb_Gain_n;
